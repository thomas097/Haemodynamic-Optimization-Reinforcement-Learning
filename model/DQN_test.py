...
def discrete_pendulum(model=None, num_episodes=10, max_timesteps=1000, render=False):
    ...
    for ep in range(num_episodes):
        ...
        for ts in range(max_timesteps):
            ...
            if model is not None:
                ...
                action = round((env.action_space.sample()[0] + 2) * 2)  # ~[0, 8] (9 actions)
...
if __name__ == '__main__':
    ...
    model = DuelingDQN(state_dim=3, num_actions=9, hidden_dims=(16, 16))

    # Sample dataset of N episodes for off-policy training
    dataset = discrete_pendulum(num_episodes=500)
    ...
    fit_dueling_double_DQN(model=model,
                           ...
                           dataset_passes=20,
                           eval_func=discrete_pendulum)  # Evaluate on pendulum simulator!

    # Try out model in simulator
    discrete_pendulum(model=model, render=True)
