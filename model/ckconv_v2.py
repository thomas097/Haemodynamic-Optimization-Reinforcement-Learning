...
class CKConv(torch.nn.Module):
    ...
    def forward(self, x, t):
        ...
        if self.training:
            ...
            self._train_max_len = torch.max(self._train_max_len, torch.max(t_max - t_min))
        ...
        z = self._causal_conv1d(x_new, weights).permute(0, 2, 1)  # -> (batch_size, seq_length, out_channels)
    ...
    def _fill_missing_observations(self, x, t):
        ...
        x_new = torch.zeros((x.shape[0], N) + x.shape[2:])
        mask = torch.zeros((x.shape[0], N), dtype=torch.bool)

        for i, t_batch in enumerate(t - t_min):  # TODO: faster!
            x_new[i, t_batch] = x[i]
            mask[i, t_batch] = 1
...
if __name__ == '__main__':
    ...
    t = torch.cumsum(torch.randint(1, 3, (B, N)), dim=1)
    x = torch.abs(torch.randn((B, N, D_in)))
    ...
    print('Time elapsed:', (time.time_ns() - time1) / (1e9 * N_repeats * B), '\n')
    print('Out:', out.shape)
