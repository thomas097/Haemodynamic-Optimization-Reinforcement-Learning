DROP TABLE IF EXISTS vasopressors_mv CASCADE;
CREATE TABLE vasopressors_mv AS
(
select 
    mimiciii.inputevents_mv.icustay_id, starttime, endtime, rate, rateuom, patientweight, orderid, linkorderid,
    ...
    CASE WHEN itemid=221906 THEN rate * 1 * patientweight -- convert to mcg/min
	 ...
	 WHEN itemid=222315 THEN rate * 5 / 60  --- /60 = convert to dose per minute 
