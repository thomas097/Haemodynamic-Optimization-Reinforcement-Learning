...
DROP TABLE IF EXISTS full_cohort CASCADE;
CREATE TABLE full_cohort AS
with serv as
(
...
select
  ...
  , case when
             t1.rn != 1
          or t1.age <= 17                                         -- keep patient >= 18
          -- or t1.first_service in ('CSURG','VSURG','TSURG')     -- do include post-surgical admissions!
