...
def evaluate_policy(policy, train_actions, dataset, state_cols, action_col, reward_col, episode_col, batch_size=128):
    ...
    for i in tqdm(range(0, len(indices), batch_size)):
        ...
        action_probs_batch = np.column_stack([np.sum(neigh_actions == a, axis=1) / k for a in range(25)])
        action_probs.append(action_probs_batch)
...
if __name__ == '__main__':
    ...
    for dataset in DATASETS:
        action_probs = evaluate_policy(policy, train_actions, dataset, STATE_COLS, ACTION_COL, REWARD_COL, EPISODE_COL)
